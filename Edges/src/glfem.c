...
void glfemDrawElement(float *x, float *y, int n)  
{
    int j;
    glColor3f(current_color[0],current_color[1],current_color[2]);
    glLineWidth(1000*current_line_width);
...
void glfemPlotEdges(femEdges *theEdges)
{
    ...
    for (i = 0; i < theEdges->nEdge; i++) {
        glfemDrawElement((float[2]){theMesh->X[theEdges->edges[i].node[0]], theMesh->X[theEdges->edges[i].node[1]]}, 
   	    	(float[2]){theMesh->Y[theEdges->edges[i].node[0]], theMesh->Y[theEdges->edges[i].node[1]]}, 2);}
}
